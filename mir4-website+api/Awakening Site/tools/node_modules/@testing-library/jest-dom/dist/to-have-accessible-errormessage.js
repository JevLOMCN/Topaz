...
function toHaveAccessibleErrorMessage(htmlElement, expectedAccessibleErrorMessage) {
  ...
  const error = (0, _utils.normalize)(((_htmlElement$ownerDoc = htmlElement.ownerDocument.getElementById(errormessageId)) == null ? void 0 : _htmlElement$ownerDoc.textContent) ?? '');
  return {
    ...
    message: () => {
      return (0, _utils.getMessage)(this, this.utils.matcherHint(method, 'element'), `Expected element ${to} have accessible error message`, expectedAccessibleErrorMessage ?? '', 'Received', error);
    }
  };
}